...
def remove_constraints(cond_indep,N,allnodes,obs):
             ...
             if any(v!=0.0 for v in  constraints_copy[0]):
                    ...
                    while k<len(shanon_cone): 
                           if constraints_copy[0]!=shanon_cone[k]: 
                               print(constraints_copy[0])
                               if (allnodes & ~obs) in nonemptysubsetsof(any(y for y,z in enumerate(constraints_copy[0] ))):
                                   print("yi")
                           ...
                           if (allnodes & ~obs) in nonemptysubsetsof(any(y for y,z in enumerate(constraints_copy[0] ))):
                               ...
                               for index, m in enumerate(constraints_copy[0]):
                                    if m!=0 and index!=0:
                                             ...
                                             entropy_cone.append(matrix_manipulate(constraints_copy[j], constraints_copy[0], index))
                                             break;                  
                           j=j+1  
...
def I_cone(par,obs,N):    #to correct
    ...
    return remove_constraints(cond_indep,N,obs,obs)       
...
def C_cone(par,allnodes,obs,N):
    ...
    to_remove_unobserved=remove_constraints(cond_indep,N,allnodes,obs)
...
a=gdag_cone([0b0,0b0,0b1,0b100],0b1111,0b1110,4)
print(a,len(a)) 
