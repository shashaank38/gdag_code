...
def advantange1(inequalities_list, variables_to_eliminate):
    count = [ count_redundant(inequalities_list, v) for v in variables_to_eliminate ]
    ...
    return variables_to_eliminate[min_index]   

def count_redundant(inequalities_list, variable):
    c = 0
    for (first,second) in itertools.combinations(inequalities_list, 2): # loop through pairs of inequalities
        if first[variable]*second[variable]<0.0:
            c=c+1
    return c
...
def faster_elimination_1(inequalities_list, variables_to_eliminate):
    for i in range(len(variables_to_eliminate)):
          ...
          unobserved_removed = Fourier_Motzkin(inequalities_list, k)
          inequalities_list=unobserved_removed[:]
...
"Testing-works fine"
inequalities=[[-1,1,1,2],[-2, -1, 1, 1], [-1,1,-1,1],[0, 0, -1, -3]]
var=[3,1]
